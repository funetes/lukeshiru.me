...
export const Logo: WrapperComponent = ({ class: className, ...properties }) => (
	<div
		class={classnames(
			...
			"pointer-events-none absolute origin-center left-1/2 top-1/2 w-200 h-200 transform-gpu -translate-y-1/2 -translate-x-1/2",
			className
		)}
		{...properties}
	>
		<svg class="overflow-visible" viewBox="0 0 80 80">
			<path
				// eslint-disable-next-line max-len
				class="motion-safe:animate-shrink-rotate origin-center transform-gpu"
				fill="#F05"
				d="M40 3.987l31.173 54.02H8.827z"
			/>
			<path
				fill="#000"
				class="motion-safe:animate-fade-in origin-center transform-gpu"
				// eslint-disable-next-line max-len
